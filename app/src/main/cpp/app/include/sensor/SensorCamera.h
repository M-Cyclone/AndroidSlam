...
private:
    ...
    AndroidImageReader                m_img_reader;                   // 解析相机的拍摄结果
    ANativeWindow*                    m_img_reader_window = nullptr;  // 存储相机拍摄结果
    mutable std::vector<AndroidImage> m_images;                       // 每帧安卓图像对象
    mutable std::vector<AImage*>      m_image_buffers;                // 对m_images的引用

    // 不需过于深入理解，以下对象用于连接安卓相机，从而获取图像
