    ...
    , m_capture_session_state_callbacks{}
{
    ...
    // Create camera device.
    {
        ...
        {
            std::cout << "[Android Slam Camera Info] Camera(ID: " << ACameraDevice_getId(device) << ") is disconnected."
                      << std::endl;
        };
        m_camera_device_state_callbacks.onError = [](void* context, ACameraDevice* device, int error)
        {
            std::cout << "[Android Slam Camera Info] Camera(ID: " << ACameraDevice_getId(device)
    ...
    // Create capture session output container.
    {
        ...
        assert(m_capture_session_output_container &&
               "[Android Slam Sensor Info] Capture Session Output Container is NULL.");
    }
    ...
    // Add image reader output to output container.
    {
        ...
        assert((cs == ACAMERA_OK) &&
               "[Android Slam Sensor Info] Failed to add ImageReaderOutput to CaptureSessionOutputContainer.");
    }
    ...
    // Create capture session.
    {
        ...
        {
            std::cout << "[Android Slam Camera Info] Session(ptr == " << session << ") is closed." << std::endl;
        };
        m_capture_session_state_callbacks.onReady = [](void* context, ACameraCaptureSession* session)
        {
            std::cout << "[Android Slam Camera Info] Session(ptr == " << session << ") is ready." << std::endl;
        };
        m_capture_session_state_callbacks.onActive = [](void* context, ACameraCaptureSession* session)
        {
            std::cout << "[Android Slam Camera Info] Session(ptr == " << session << ") is activated." << std::endl;
        };
    ...
    // Add target to capture request.
    {
        ...
        assert((cs == ACAMERA_OK) &&
               "[Android Slam Sensor Info] Failed to add camera output target to capture request.");
    }
}
...
void SensorCamera::startCapture()
{
    ...
    camera_status_t               cs = ACameraCaptureSession_setRepeatingRequest(m_capture_session.get(),
                                                                   nullptr,
