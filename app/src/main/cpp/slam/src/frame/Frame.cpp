...
void Frame::copyFrom(const Frame& rhs) noexcept
{
    *this = rhs;
    this->mK = rhs.mK.clone();
    this->mK_ = Converter::toMatrix3f(rhs.mK);
    this->mDistCoef = rhs.mDistCoef.clone();
    ...
    if(rhs.mbHasPose)
    {
        this->SetPose(rhs.GetPose());
    }

    if(rhs.HasVelocity())
    {
        this->SetVelocity(rhs.GetVelocity());
    }

    this->mmProjectPoints = rhs.mmProjectPoints;
    this->mmMatchedInImage = rhs.mmMatchedInImage;
}
