...
namespace ORB_SLAM3 {
    ...
    Settings::Settings(const SettingDesc& desc)
        : sensor_(desc.sensor)
        ...
        , bNeedToResize2_(false)
    {
        // read camera 1
        {
            ...
            if(desc.cameraInfo.cameraType == CameraType::PinHole)
            {
                cameraType_ = CameraType::PinHole;

                vCalibration = { desc.cameraInfo.fx, desc.cameraInfo.fy, desc.cameraInfo.cx, desc.cameraInfo.cy };
                calibration1_ = new Pinhole(vCalibration);
                originalCalib1_ = new Pinhole(vCalibration);


                if(desc.distortion.has_value())
                {
                    vPinHoleDistorsion1_.push_back(desc.distortion->k1);
                    vPinHoleDistorsion1_.push_back(desc.distortion->k2);
                    vPinHoleDistorsion1_.push_back(desc.distortion->p1);
                    vPinHoleDistorsion1_.push_back(desc.distortion->p2);

                    if(desc.distortion->k3.has_value())
                    {
                        vPinHoleDistorsion1_.push_back(*desc.distortion->k3);
                    }
                }
            ...
            {
                assert(false && "Android Slam does not support these camera type for now.");
            }
        }

        // read camera 2 is not supported for now
        {
            if(sensor_ == System::STEREO || sensor_ == System::IMU_STEREO)
            {
                assert(false && "Android Slam only supports mono camera case.");
            }
        }

        // image info
        {
            originalImSize_.width = desc.imageInfo.width;
            originalImSize_.height = desc.imageInfo.height;

            if(desc.imageInfo.newHeight.has_value())
            {
                bNeedToResize1_ = true;
                newImSize_.height = *desc.imageInfo.newHeight;
            ...
            if(desc.imageInfo.newWidth.has_value())
            {
                bNeedToResize1_ = true;
                newImSize_.width = *desc.imageInfo.newWidth;
...
//
            ...
            fps_ = desc.imageInfo.fps;
            bRGB_ = desc.imageInfo.bRGB;
        }
        ...
        if(sensor_ == System::IMU_MONOCULAR || sensor_ == System::IMU_STEREO || sensor_ == System::IMU_RGBD)
        {
            ...
            gyroWalk_ = desc.imuInfo.gyroWalk;
            accWalk_ = desc.imuInfo.accWalk;
            imuFrequency_ = desc.imuInfo.frequency;
            Tbc_ = Converter::toSophus(desc.imuInfo.cvTbc);
            insertKFsWhenLost_ = desc.imuInfo.bInsertKFsWhenLost;
        }

        // read rgbd is not supported for now.
        if (sensor_ == System::RGBD || sensor_ == System::IMU_RGBD)
        {
            assert(false && "Read rgbd is not supported for now.");
        }

        // read orb
        {
            nFeatures_ = desc.orbInfo.nFeatures;
            scaleFactor_ = desc.orbInfo.scaleFactor;
            nLevels_ = desc.orbInfo.nLevels;
            initThFAST_ = desc.orbInfo.initThFAST;
            minThFAST_ = desc.orbInfo.minThFAST;
        }

        // read viewer
        {
            keyFrameSize_ = desc.viewerInfo.keyframeSize;
            keyFrameLineWidth_ = desc.viewerInfo.keyframeLineWidth;
            graphLineWidth_ = desc.viewerInfo.graphLineWidth;
            pointSize_ = desc.viewerInfo.pointSize;
            cameraSize_ = desc.viewerInfo.cameraSize;
            cameraLineWidth_ = desc.viewerInfo.cameraLineWidth;
            viewPointX_ = desc.viewerInfo.viewPointX;
            viewPointY_ = desc.viewerInfo.viewPointY;
            viewPointZ_ = desc.viewerInfo.viewPointZ;
            viewPointF_ = desc.viewerInfo.viewPointF;
            imageViewerScale_ = desc.viewerInfo.imageViewerScale;
        }

        // load atlas
        {
            sLoadFrom_ = desc.atlasInfo.loadPath;
            sSaveto_ = desc.atlasInfo.savePath;
        }

        // other info
        {
            thFarPoints_ = desc.otherInfo.thFarPoints;
        }
