    ...
    mbActivateLocalizationMode(false), mbDeactivateLocalizationMode(false), mbShutDown(false)
{
    ...
    if(!node.empty() && node.isString() && node.string() == "1.0"){
        ...
        cout << "Settings is set up from the setting file." << endl;
        cout << (*settings_) << endl;
    }
    else
    {
        settings_ = nullptr;
        cout << "Settings is set to nullptr." << endl;
...
System::System(ORBVocabulary* vocabulary, Settings* settings, const eSensor sensor)
    : mSensor(sensor)
    ...
    , settings_(settings)
{
    ...
    {
        // create tracker
        mpTracker = new Tracking(this, mpVocabulary, mpAtlas, mpKeyFrameDatabase, "", mSensor, settings_, "");
        ...
        bool bImu = (mSensor == IMU_MONOCULAR) || (mSensor == IMU_STEREO) || (mSensor == IMU_RGBD);
        ...
        mpLoopCloser = new LoopClosing(mpAtlas, mpKeyFrameDatabase, mpVocabulary, mSensor != MONOCULAR, bActiveLoopClosing); // mSensor!=MONOCULAR);
        mptLoopClosing = new thread(&ORB_SLAM3::LoopClosing::Run, mpLoopCloser);
